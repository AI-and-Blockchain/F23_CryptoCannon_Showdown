(function () {
  ...
  Game.prototype.startGame = function (e) {
    ...
    player_ship_pos = self.humanFleet.fleetRoster.reverse().map((ship) => {
      return [ship.xPosition * 10 + ship.yPosition, ship.direction == 0];
    });

    computer_ship_pos = self.computerFleet.fleetRoster.reverse().map((ship) => {
      console.log(ship);
      return [ship.xPosition * 10 + ship.yPosition, ship.direction == 0];
    });

    const response = fetch("http://127.0.0.1:5000/setup_board", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        user_board: player_ship_pos,
        ai_board: computer_ship_pos,
      }),
    });
